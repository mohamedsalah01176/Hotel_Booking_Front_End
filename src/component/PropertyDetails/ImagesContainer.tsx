...
const ImagesContainer = ({images}:{images:string[]}) => {
  ...
  if (showAll) {
    return (
      <ShowAllImages images={images} setShowAll={setShowAll}/>
    );
  }
  return (
    <div className=" flex flex-col md:flex-row justify-between items-start my-5 overflow-hidden rounded-3xl relative">
      <button onClick={()=>setShowAll(true)} className="bg-[#02717e] px-5 py-2.5 text-white rounded-xl absolute bottom-7 right-8 z-10 cursor-pointer hover:bg-[#e77008] transition-all duration-300">Show All</button>
